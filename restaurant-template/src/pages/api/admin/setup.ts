 ...
 * POST /api/admin/setup  with  { secret: "rc-setup-2026" }
 *
 * Applies all three migrations to the Supabase database using the
 * Management API (requires SUPABASE_ACCESS_TOKEN env var).
 *
 * DELETE THIS FILE after the database is set up.
 */
import type { APIRoute } from 'astro';

export const prerender = false;

// SQL for all three migrations concatenated
const MIGRATION_SQL = `
-- ═══════════════════════════════════════════
-- 0001_init: schema
-- ═══════════════════════════════════════════
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='menu_items' and policyname='menu readable') then
    create policy "menu readable" on public.menu_items for select using (is_available = true or public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='menu_categories' and policyname='categories readable') then
    create policy "categories readable" on public.menu_categories for select using (is_active = true or public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='loyalty_rewards' and policyname='rewards readable') then
    create policy "rewards readable" on public.loyalty_rewards for select using (is_active = true or public.is_admin());
  end if;
  ...
  if not exists (select 1 from pg_policies where tablename='orders' and policyname='create order') then
    create policy "create order" on public.orders for insert with check (auth.uid() = user_id or user_id is null);
  end if;
  if not exists (select 1 from pg_policies where tablename='reservations' and policyname='own reservations') then
    create policy "own reservations" on public.reservations for select using (auth.uid() = user_id or public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='reservations' and policyname='create reservation') then
    create policy "create reservation" on public.reservations for insert with check (true);
  end if;
  if not exists (select 1 from pg_policies where tablename='reservations' and policyname='admin manage reservations') then
    create policy "admin manage reservations" on public.reservations for update using (public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='menu_items' and policyname='admin menu write') then
    create policy "admin menu write" on public.menu_items for all using (public.is_admin()) with check (public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='menu_categories' and policyname='admin categories write') then
    create policy "admin categories write" on public.menu_categories for all using (public.is_admin()) with check (public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='loyalty_rewards' and policyname='admin rewards write') then
    create policy "admin rewards write" on public.loyalty_rewards for all using (public.is_admin()) with check (public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where tablename='loyalty_transactions' and policyname='own loyalty') then
    create policy "own loyalty" on public.loyalty_transactions for select using (auth.uid() = user_id or public.is_admin());
  end if;
end $$;
...
create table if not exists public.promo_codes (
  ...
  discount_type text not null check (discount_type in ('percent','cents')),
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='promo_codes' and policyname='promo codes readable') then
    create policy "promo codes readable" on public.promo_codes for select using (is_active = true and (expires_at is null or expires_at > now()));
  end if;
end $$;
...
insert into menu_items (id,name,description,price_cents,category_id,dietary_tags,is_available,is_featured) values
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (body.secret !== 'rc-setup-2026') return json({ error: 'forbidden' }, 403);

  const accessToken = import.meta.env.SUPABASE_ACCESS_TOKEN;
  if (!accessToken) {
    return json({
      error: 'SUPABASE_ACCESS_TOKEN not set.',
      instructions: 'Add your Supabase Personal Access Token as SUPABASE_ACCESS_TOKEN in Replit Secrets. Generate one at https://app.supabase.com/account/tokens',
    }, 400);
  }

  const projectRef = new URL(import.meta.env.PUBLIC_SUPABASE_URL).hostname.split('.')[0];
  ...
  if (!res.ok) return json({ error: 'Supabase Management API error', detail: result }, res.status);
  return json({ success: true, message: 'All migrations applied + 27 menu items seeded!', result });
};

export const GET: APIRoute = async () => {
  return new Response(JSON.stringify({
    usage: 'POST /api/admin/setup with body { "secret": "rc-setup-2026" }',
    note: 'Requires SUPABASE_ACCESS_TOKEN env var (personal access token from https://app.supabase.com/account/tokens)',
